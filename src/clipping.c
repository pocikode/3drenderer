...
void clip_polygon_against_plane(polygon_t *polygon, int plane)
{
  ...
  while (current_vertex != &polygon->vertices[polygon->num_vertices])
  {
    ...
    // if changing from inside to outside or vice versa, compute intersection
    if (current_dot * previous_dot < 0)
    {
      // interpolation factor: t = Q1/(Q1 - Q2)
      float t = previous_dot / (previous_dot - current_dot);

      // calculate intersection point I = Q1 + t(Q2 - Q1) or I = Qp + t(Qc - Qp)
      vec3_t intersection = {
      ...
      // calculate interpolated texture coordinate
      ...
      inside_vertices[num_inside_vertices] = intersection;
